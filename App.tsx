...
import { 
  ...
  Mail, Lock, Building2, Phone, Search, Hash, TrendingUp, TrendingDown, Bell, Clock, FileSpreadsheet
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const deleteInvoice = (id: string) => {
    if (window.confirm("Delete this invoice permanently? This will remove related outstanding balances.")) {
    ...
    setMasterItems(prev => prev.map(item => {
      if (item.id === itemId) {
        const newHistory: PriceHistoryEntry = {
          ...
          variance: alertData.amountChange,
          percentChange: alertData.change,
          source: 'audit',
          invoiceNumber: alertData.invoiceNumber
        };
        ...
        return {
          ...item,
          currentPrice: alertData.newPrice,
          history: [newHistory, ...item.history],
          lastUpdated: alertData.date
        };
      }
      return item;
    }));
    ...
    enrichedInvoices.forEach(inv => {
      inv.items.forEach(item => {
        if (item.priceChange && Math.abs(item.priceChange) > 0.01) {
          const alreadyAccepted = acceptedAlertHistory.some(h => 
            h.invoiceNumber === inv.invoiceNumber && h.itemName === item.name
          );
          const newerAcceptedExists = acceptedAlertHistory.some(h => 
            h.itemName === item.name && 
            h.supplier === inv.supplierName && 
            new Date(h.date) > new Date(inv.date)
          );
        ...
        <div className="w-full max-w-md relative z-10">
           ...
           <div className="bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-10">
              ...
              <form onSubmit={handleVaultAccess} className="space-y-6">
                 ...
                 <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest">Enter Vault</button>
              </form>
           </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 font-sans text-slate-900 relative">
      <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[1000] space-y-2 w-full max-w-md px-4 pointer-events-none">
         {toasts.map(t => (
           <div key={t.id} className={`p-4 rounded-2xl shadow-2xl border flex items-start space-x-3 pointer-events-auto ${t.type === 'success' ? 'bg-emerald-600' : t.type === 'warning' ? 'bg-amber-600' : 'bg-rose-600'} text-white`}>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
           ...
           {activeAlerts.length > 0 && (
              <div onClick={() => setActiveTab('alerts')} className="bg-rose-50 border border-rose-200 px-4 py-2 rounded-xl flex items-center space-x-3 animate-pulse cursor-pointer">
                ...
                <span className="text-[10px] font-black uppercase text-rose-700">{activeAlerts.length} New Price Shifts</span>
              </div>
           )}
        </div>
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 auto-rows-min">
                ...
                <div className="sm:col-span-2 bg-slate-900 rounded-[2rem] p-8 text-white">
                  <h3 className="font-black text-xs uppercase mb-6 flex items-center"><Bell size={18} className="mr-3 text-rose-500" /> Quick Alert Preview</h3>
                    ...
                    {activeAlerts.slice(0, 3).map(alert => (
                      <div key={alert.id} className="flex justify-between items-center p-4 bg-white/5 rounded-2xl">
                        <div>
                          ...
                          <h4 className="text-[10px] font-black uppercase">{alert.itemName}</h4>
                        </div>
                        <div className="text-right">
                          <p className={`text-xs font-black ${alert.change > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                            {alert.change > 0 ? '+' : ''}{alert.change.toFixed(1)}%
                          </p>
                          <button onClick={() => setActiveTab('alerts')} className="text-[9px] font-black uppercase text-blue-400 mt-1">Audit Hub</button>
                        </div>
                      </div>
                    ))}
            ...
            <div className="space-y-10">
              <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl overflow-hidden relative">
                <h3 className="font-black uppercase text-sm tracking-widest flex items-center mb-10 text-rose-400">
                  <Bell size={24} className="mr-4" /> Live Price Alert Hub
                </h3>
                {activeAlerts.length > 0 ? (
                  ...
                  <div className="py-24 text-center">
                    ...
                    <p className="text-slate-500 font-bold uppercase text-[11px] mt-2">No pending price variances found.</p>
                  </div>
                )}
              </div>

              <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-10">
                <h3 className="font-black uppercase text-xs tracking-widest text-slate-400 mb-8">Accepted Adjustment Log</h3>
                <div className="space-y-4">
                  {acceptedAlertHistory.map((h, i) => (
                    <div key={i} className="flex justify-between items-center p-5 bg-slate-50 rounded-2xl border border-slate-100">
                      <div>
                        ...
                        <p className="text-[9px] font-bold text-slate-500 uppercase">{h.supplier} • Accepted {new Date(h.acceptedAt).toLocaleDateString()}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[11px] font-black text-emerald-600 uppercase">${h.newPrice.toFixed(2)}</p>
                        <p className="text-[9px] font-bold text-slate-400 uppercase">Inv {h.invoiceNumber}</p>
                      </div>
                    </div>
                  ))}
                  {acceptedAlertHistory.length === 0 && <p className="text-[11px] font-bold text-slate-400 text-center py-12 uppercase">No historical records</p>}
                </div>
              </div>
            </div>
          )}
                          ...
                          <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="px-6 py-6">
                               <div className="flex items-center justify-center space-x-2">
                                  <button onClick={() => toggleInvoicePaid(inv.id)} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase border transition-all ${inv.isPaid ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-400 border-slate-200 hover:border-emerald-500'}`}>Paid</button>
                                  <button onClick={() => toggleInvoiceHold(inv.id)} className="px-3 py-1.5 rounded-lg text-[9px] font-black uppercase border border-slate-200 bg-white text-slate-400 hover:text-rose-600 transition-all">Move to Hold</button>
                               </div>
                            </td>
               ...
               <div className="p-6 border-b flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-3">
                    <button 
                      ...
                      className={`flex items-center space-x-2 px-6 py-3 rounded-2xl text-[10px] font-black uppercase transition-all ${selectedStockItems.length > 0 ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
                    >
                     ...
                     <thead>
                        <tr className="bg-slate-50 border-b text-[10px] font-black uppercase text-slate-400">
                           <th className="px-6 py-5 w-10">
                              <input 
                                type="checkbox" 
                                checked={selectedStockItems.length === masterItems.length && masterItems.length > 0}
                                onChange={(e) => {
                                  if (e.target.checked) setSelectedStockItems(masterItems.map(m => m.id));
                                  else setSelectedStockItems([]);
                                }}
                          ...
                          <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                            <td className="px-6 py-6">
                              <input 
                                ...
                                onChange={() => {
                                  if (selectedStockItems.includes(item.id)) setSelectedStockItems(prev => prev.filter(i => i !== item.id));
                                  else setSelectedStockItems(prev => [...prev, item.id]);
                                }}
                                className="rounded border-slate-300"
                              />
                            </td>
                            ...
                            <td className="px-6 py-6 text-center">
                               <button onClick={() => deleteMasterItem(item.id)} className="p-2 text-slate-300 hover:text-rose-600 transition-colors">
                                 <Trash2 size={16} />
                               </button>
                            </td>
                          </tr>
          ...
          {activeTab === 'suppliers' && (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
               {suppliers.map(sup => {
                 // Calculate vendor intelligence: outstanding is only from existing invoices
                 const outstanding = rawInvoices
                  .filter(inv => inv.supplierName === sup.name && !inv.isPaid && !inv.isHold)
                 ...
                 return (
                  <div key={sup.id} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8 space-y-6 hover:shadow-lg transition-all group">
                      <div className="flex items-start justify-between">
                        ...
                        {outstanding > 0 && (
                          <div className="bg-amber-50 text-amber-600 px-3 py-1 rounded-full text-[9px] font-black uppercase">Outstanding</div>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="bg-slate-50 p-4 rounded-2xl">
                            ...
                            <p className={`font-black text-xs ${outstanding > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
               ...
               {suppliers.length === 0 && (
                 <div className="col-span-full py-24 text-center">
                    ...
                    <h4 className="font-black text-slate-400 uppercase">No Vendor Intelligence Found</h4>
                    <p className="text-slate-300 font-bold uppercase text-[10px] mt-2">Upload invoices to build vendor profiles.</p>
                 </div>
               )}
            </div>
          )}
