...
import { 
  ...
  Mail, Lock, Building2, Phone, Search, Hash, TrendingUp, TrendingDown, Bell, Clock
} from 'lucide-react';
...
const App: React.FC = () => {
    ...
    enrichedInvoices.forEach(inv => {
      inv.items.forEach(item => {
        if (item.priceChange && Math.abs(item.priceChange) > 0.01) {
          // Check if this specific alert (inv + item) has been accepted already
          const alreadyAccepted = acceptedAlertHistory.some(h => 
            h.invoiceNumber === inv.invoiceNumber && h.itemName === item.name
          );

          // Check if there's a NEWER accepted price for this same item
  ...
  const stats = useMemo(() => {
    ...
    return { 
      totalPayable, 
      payableBySupplier: Object.entries(payableBySupplier).sort((a, b) => b[1] - a[1]),
      totalGst, 
        ...
        <div className="space-y-8">
                     ...
                     <tbody className="divide-y divide-slate-100">
                        {masterItems.map(item => (
                          <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="px-6 py-6 text-center">
                               <span className="text-[9px] font-black uppercase text-slate-300 tracking-widest px-3 py-1 bg-slate-100 rounded-full">Anchored</span>
                            </td>
                          </tr>
                        ))}
                     </tbody>
          ...
          {activeTab === 'suppliers' && (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
               {suppliers.map(sup => (
                 <div key={sup.id} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6 space-y-4">
                    <div className="flex items-center justify-between border-b pb-4">
                       <h3 className="font-black text-slate-900 uppercase text-sm truncate">{sup.name}</h3>
                    </div>
