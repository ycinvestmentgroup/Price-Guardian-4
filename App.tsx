...
const App: React.FC = () => {
  ...
  // Helper to reset state for new users or logout
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (fbUser: FirebaseUser | null) => {
      if (fbUser) {
        // Load data from unique user document
  ...
  // Automated Sync Engine - Per-User Document logic
  useEffect(() => {
    if (auth.currentUser && dataLoaded) {
      const syncToCloud = async () => {
        setIsSyncing(true);
        try {
          // Strict user isolation via UID path
  ...
  const handleVaultAccess = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setUploadProgress(authMode === 'login' ? "Opening Vault..." : "Initializing New Database...");
    try {
      if (authMode === 'login') {
        await signInWithEmailAndPassword(auth, loginEmail, loginPass);
        addToast("Vault secured.", "success");
      } else {
        // Reset everything before creating a new user to ensure a clean database
        resetLocalData();
        await createUserWithEmailAndPassword(auth, loginEmail, loginPass);
        addToast("New isolated database created.", "success");
      }
    } catch (err: any) {
  ...
  // FIX: Added handleFileUpload to process selected files and use Gemini for extraction
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        setSuppliers(prev => {
          ...
          if (exists) {
            return prev.map(s => s.name === newInvoice.supplierName ? { ...s, totalSpent: s.totalSpent + newInvoice.totalAmount } : s);
          }
          return [...prev, {
            id: Math.random().toString(36).substring(2, 9),
      ...
      addToast("Documents audited and verified.", "success");
  ...
  // FIX: Added handleAcceptPrice to process and commit price alerts
  ...
  const commitPriceChange = (itemId: string, alertData: any) => {
    ...
    addToast("Master registry updated", "success");
  };
  ...
  // Main UI components remain consistent, now tied to isolated states...
  return (
             ...
             <div className="bg-slate-800/50 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-10 text-left shadow-2xl">
                ...
                <form onSubmit={handleVaultAccess} className="space-y-6">
                   <div className="relative">
                      ...
                      <input type="email" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} required className="w-full bg-slate-900/50 border border-white/5 rounded-2xl py-4 pl-12 text-white text-xs outline-none focus:border-blue-500/50 transition-all" placeholder="Email Address" />
                   </div>
                   <div className="relative">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600" size={18} />
                      <input type="password" value={loginPass} onChange={e => setLoginPass(e.target.value)} required className="w-full bg-slate-900/50 border border-white/5 rounded-2xl py-4 pl-12 text-white text-xs outline-none focus:border-blue-500/50 transition-all" placeholder="Password" />
                   </div>
                   <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest transition-all hover:bg-blue-500 shadow-xl active:scale-95">{authMode === 'login' ? 'Enter Vault' : 'Create Isolated Database'}</button>
                </form>
             </div>
             <p className="mt-8 text-slate-500 text-[10px] font-bold uppercase tracking-widest">Strict Multi-Tenant Encryption & Isolation</p>
          </div>
        </div>
      ) : (
        <>
          <nav className={`fixed lg:sticky lg:top-0 inset-y-0 left-0 w-72 bg-slate-900 text-slate-400 flex flex-col shrink-0 z-[100] transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} h-screen overflow-y-auto shadow-2xl`}>
            ...
            <div className="p-6 mt-auto">
               <div className="p-4 bg-slate-800/30 rounded-2xl mb-4 border border-white/5">
                  <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Authenticated As</p>
                  <p className="text-[10px] font-bold text-white truncate uppercase">{currentUser.email}</p>
               </div>
          ...
          <main className="flex-1 p-4 lg:p-10 max-w-full overflow-x-hidden min-h-screen">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
               ...
               {activeAlerts.length > 0 && (
                  <div onClick={() => setActiveTab('alerts')} className="bg-rose-50 border border-rose-200 px-4 py-2 rounded-xl flex items-center space-x-3 animate-pulse cursor-pointer shadow-sm">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
                      <h4 className="text-[10px] font-black uppercase text-slate-400 mb-4 tracking-widest">Supplier Outstanding</h4>
                      <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar">
                        ...
                        {stats.payableBySupplier.length === 0 && <p className="text-[10px] font-bold text-slate-400 text-center py-6">All clear</p>}
                      </div>
                    </div>
                  </div>
                  <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 auto-rows-min">
                    <StatCard label="Database Audits" value={stats.totalCount} icon={Files} color="slate" />
                    <StatCard label="GST Tracker (YTD)" value={`$${stats.totalGst.toFixed(2)}`} icon={Banknote} color="emerald" />
                    <StatCard label="Registered Vendors" value={stats.supplierCount} icon={Building2} color="blue" />
...
// Sub-components for cleaned up tabs
...
const AlertsHubView = ({ activeAlerts, handleAcceptPrice, acceptedAlertHistory }: any) => (
  <div className="space-y-10">
    <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
      <h3 className="font-black uppercase text-sm tracking-widest flex items-center mb-10 text-rose-400"><Bell size={24} className="mr-4" /> Price Audit Pulse</h3>
      {activeAlerts.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {activeAlerts.map((alert: any) => (
            <div key={alert.id} className="flex flex-col p-8 bg-white/5 rounded-[2rem] border border-white/10 hover:border-rose-500/50 transition-all">
              ...
              <button onClick={() => handleAcceptPrice(alert)} className="w-full bg-blue-600 hover:bg-emerald-600 text-white py-4 rounded-2xl text-[11px] font-black uppercase transition-all">Commit Adjustment</button>
            </div>
          ))}
        </div>
      ) : (
        <div className="py-24 text-center"><ShieldCheck size={64} className="mx-auto text-emerald-500/50 mb-6" /><h4 className="font-black text-white uppercase">Vault Prices Optimized</h4></div>
      )}
    </div>
  </div>
);
...
const VendorsView = ({ suppliers, rawInvoices }: any) => (
  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
     {suppliers.map((sup: any) => {
       const outstanding = rawInvoices.filter((inv: any) => inv.supplierName === sup.name && !inv.isPaid && !inv.isHold).reduce((sum: number, inv: any) => sum + inv.totalAmount, 0);
       return (
        <div key={sup.id} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8 space-y-6">
            <div className="flex items-center space-x-4">
                ...
                <h3 className="font-black text-slate-900 uppercase text-sm">{sup.name}</h3>
            </div>
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-2xl"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Total Volume</p><p className="font-black text-slate-900 text-xs">${sup.totalSpent.toLocaleString()}</p></div>
                <div className="bg-slate-50 p-4 rounded-2xl"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Outstanding</p><p className={`font-black text-xs ${outstanding > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>${outstanding.toFixed(2)}</p></div>
            </div>
        </div>
       );
     })}
        ...
        return (
          <div key={month} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden mb-8">
            <div className="px-8 py-6 bg-slate-50 border-b flex justify-between items-center">
              <h4 className="font-black text-slate-900 uppercase text-sm">{month} Compliance</h4>
              <div className="bg-emerald-600 text-white px-4 py-1.5 rounded-xl text-[10px] font-black">Settled: ${(monthlyTotal as number).toFixed(2)}</div>
            </div>
            <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-4">
              {Object.entries(supplierGstMap).map(([name, amount]: any) => (<div key={name} className="p-5 bg-slate-50 rounded-2xl"><p className="text-[9px] font-black text-slate-400 uppercase mb-2 truncate">{name}</p><span className="font-black text-slate-900 text-lg">${amount.toFixed(2)}</span></div>))}
            </div>
          </div>
        );
      })}
  ...
  <div className="fixed inset-0 z-[1000] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
       <div className="p-8 bg-slate-900 text-white flex justify-between items-start">
          <div><h2 className="text-2xl font-black uppercase tracking-tighter">{selectedInvoice.supplierName}</h2><p className="text-xs font-bold text-blue-400 mt-1">Invoice #{selectedInvoice.invoiceNumber}</p></div>
          <button onClick={() => setSelectedInvoice(null)} className="p-2 bg-white/10 rounded-xl"><X size={20} /></button>
       </div>
       <div className="p-8 overflow-y-auto custom-scrollbar flex-1 space-y-8">
          <div className="grid grid-cols-2 gap-4">
             <div className="p-4 bg-slate-50 rounded-2xl"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Total</p><p className="font-black text-xl text-slate-900">${selectedInvoice.totalAmount.toFixed(2)}</p></div>
             <div className="p-4 bg-slate-50 rounded-2xl"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">GST</p><p className="font-black text-xl text-emerald-600">${selectedInvoice.gstAmount.toFixed(2)}</p></div>
          </div>
          <div className="space-y-2">
            {selectedInvoice.items.map((item: any, idx: number) => (
              <div key={idx} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl">
                <div><p className="text-[11px] font-black text-slate-900 uppercase">{item.name}</p><p className="text-[9px] font-bold text-slate-400 uppercase">{item.quantity} x ${item.unitPrice.toFixed(2)}</p></div>
