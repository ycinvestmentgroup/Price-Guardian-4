    ...
    if (auth.currentUser && dataLoaded) {
      const syncToCloud = async () => {
        ...
        try {
          ...
          // CRITICAL FIX: Ensure no undefined values are sent to Firestore
      ...
      <div className="p-8 border-b flex justify-between items-center">
        <div>
          <h3 className="font-black text-slate-900 uppercase text-sm tracking-tight flex items-center">
            <History size={18} className="mr-3 text-slate-400" /> Adjustment History
          </h3>
          <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Audit log of all accepted price shifts</p>
        </div>
        ...
        <table className="w-full text-left min-w-[1000px]">
          <thead>
            <tr className="bg-slate-50 border-b text-[10px] font-black uppercase text-slate-400">
              ...
              <th className="px-8 py-5">Source Invoice</th>
              <th className="px-8 py-5 text-right">Adjusted Rate</th>
              <th className="px-8 py-5 text-right">Shift %</th>
            ...
            {acceptedAlertHistory.map((entry: any, idx: number) => (
              <tr key={idx} className="hover:bg-slate-50 transition-colors">
                <td className="px-8 py-6">
                  <p className="text-[10px] font-black text-slate-900">{entry.date}</p>
                  <p className="text-[9px] font-bold text-slate-400 mt-0.5 uppercase">Accepted {new Date(entry.acceptedAt).toLocaleDateString()}</p>
                </td>
                <td className="px-8 py-6">
                ...
                <td className="px-8 py-6">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-[10px] font-bold text-slate-500">{entry.invoiceNumber}</span>
                  </div>
                </td>
                <td className="px-8 py-6 text-right">
                  <p className="text-[11px] font-black text-slate-900">${entry.newPrice.toFixed(2)}</p>
                  <p className="text-[9px] font-bold text-slate-400 mt-0.5 uppercase">from ${entry.oldPrice.toFixed(2)}</p>
                </td>
                <td className="px-8 py-6 text-right">
                ...
                <td className="px-8 py-6 text-center">
                  <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-lg text-[9px] font-black uppercase border border-emerald-100">Anchored</span>
                </td>
              </tr>
            ))}
            {acceptedAlertHistory.length === 0 && (
              <tr>
                <td colSpan={6} className="py-24 text-center">
                  ...
                  <p className="text-[10px] font-black text-slate-300 uppercase">No adjustment records in database.</p>
                </td>
              </tr>
            )}
          </tbody>
            ...
            <div className="space-y-4 py-2">
               ...
               <div className="p-4 bg-slate-900 rounded-2xl border border-slate-800 text-white">
                  <div className="flex items-start space-x-3">
                     ...
                     <div className="min-w-0">
                        <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Settlement Account (EFT)</p>
               ...
               <div className="pt-2">
                  <div className="flex items-start space-x-3">
                     ...
                     <div className="min-w-0">
                        <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Registered Address</p>
  ...
  <div className="fixed inset-0 z-[1000] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
    <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95">
       ...
       <div className="p-8 overflow-y-auto custom-scrollbar flex-1 space-y-8">
          <div className="grid grid-cols-2 gap-4">
             <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Total Payable</p><p className="font-black text-xl text-slate-900">${selectedInvoice.totalAmount.toFixed(2)}</p></div>
             <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">GST Credit</p><p className="font-black text-xl text-emerald-600">${(selectedInvoice.gstAmount || 0).toFixed(2)}</p></div>
          </div>
          <div className="space-y-2">
            {selectedInvoice.items.map((item: any, idx: number) => (
              <div key={idx} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                <div className="min-w-0"><p className="text-[11px] font-black text-slate-900 uppercase truncate">{item.name}</p><p className="text-[9px] font-bold text-slate-400 uppercase">{item.quantity} units x ${item.unitPrice.toFixed(2)}</p></div>
