...
import { 
  ...
  Mail, Lock, Building2, Phone, Search, Hash, TrendingUp, TrendingDown, Bell, Clock, FileSpreadsheet, Eye, Undo2, MapPin, Download
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        const normalizedSupplierName = extractedData.supplierName.trim().toUpperCase();
        ...
        setSuppliers(prev => {
          const exists = prev.find(s => 
            s.name.toUpperCase() === normalizedSupplierName || 
            (extractedData.abn && s.abn === extractedData.abn)
          );
          
          if (exists) {
            return prev.map(s => (s.name.toUpperCase() === normalizedSupplierName || (extractedData.abn && s.abn === extractedData.abn)) ? { 
              ...s, 
        ...
        setMasterItems(prev => {
          ...
          newInvoice.items.forEach(item => {
            const masterIdx = updated.findIndex(m => m.name === item.name && m.supplierName === normalizedSupplierName);
            if (masterIdx === -1) {
              updated.push({
                id: Math.random().toString(36).substring(2, 9),
                supplierName: normalizedSupplierName,
      ...
      addToast(err.message || "Audit service unavailable.", "error");
    } finally {
      setLoading(false);
      setUploadProgress('');
    }
  ...
  const exportAuditTrailToCSV = () => {
    if (acceptedAlertHistory.length === 0) return;
    const header = "Date,Item,Supplier,Old Price,New Price,Variance %,Invoice #,Accepted At\n";
    const csvContent = acceptedAlertHistory.map(h => 
      `"${h.date}","${h.itemName}","${h.supplier}",${h.oldPrice},${h.newPrice},${h.change.toFixed(2)},"${h.invoiceNumber}","${h.acceptedAt}"`
    ).join("\n");
    const blob = new Blob([header + csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `Audit_Trail_Export_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  const stats = useMemo(() => {
    ...
    const totalOnHold = enrichedInvoices.filter(i => i.isHold).reduce((sum, i) => sum + i.totalAmount, 0);
    
    const payableBySupplier = unpaid.reduce((acc: Record<string, number>, inv) => {
      const name = inv.supplierName.trim().toUpperCase();
        ...
        <>
          ...
          <main className="flex-1 p-4 lg:p-10 max-w-full overflow-x-hidden min-h-screen">
            ...
            <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
                      <h4 className="text-[10px] font-black uppercase text-slate-400 mb-4 tracking-widest text-left">Supplier Summary</h4>
                      <div className="space-y-3">
                        {stats.payableBySupplier.slice(0, 10).map(([name, amount]) => (
                        ...
                        {stats.payableBySupplier.length === 0 && (
                          <p className="py-4 text-center text-[10px] font-bold text-slate-300 uppercase tracking-widest">All accounts settled</p>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              {activeTab === 'hold' && <HoldView enrichedInvoices={enrichedInvoices} toggleInvoiceHold={toggleInvoiceHold} totalOnHold={stats.totalOnHold} />}
              {activeTab === 'alerts' && <AlertsHubView activeAlerts={activeAlerts} handleAcceptPrice={handleAcceptPrice} acceptedAlertHistory={acceptedAlertHistory} exportTrail={exportAuditTrailToCSV} />}
              {activeTab === 'stock' && <MasterStockView masterItems={masterItems} selectedStockItems={selectedStockItems} setSelectedStockItems={setSelectedStockItems} exportStockToCSV={exportStockToCSV} deleteMasterItem={deleteMasterItem} searchQuery={stockSearchQuery} setSearchQuery={setStockSearchQuery} />}
...
const AlertsHubView = ({ activeAlerts, handleAcceptPrice, acceptedAlertHistory, exportTrail }: any) => (
  <div className="space-y-10 text-left">
    <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden text-left">
      <div className="flex justify-between items-center mb-10">
        <h3 className="font-black uppercase text-sm tracking-widest flex items-center text-rose-400 text-left">
      ...
      {activeAlerts.length > 0 ? (
            ...
            <div key={alert.id} className="flex flex-col p-8 bg-white/5 rounded-[2rem] border border-white/10 hover:border-rose-500/50 transition-all group">
              <div className="flex justify-between items-start mb-6">
                <div className="min-w-0">
                  <p className="text-[10px] font-black text-slate-500 uppercase mb-1 text-left">{alert.supplier}</p>
                  <h4 className="font-black text-white text-lg uppercase leading-tight truncate text-left">{alert.itemName}</h4>
                  <p className="text-[10px] font-bold text-slate-500 mt-2 text-left">Detected in Inv #{alert.invoiceNumber}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8 text-left">
                <div className="bg-black/20 p-4 rounded-2xl">
                  <p className="text-[9px] font-black text-slate-500 uppercase mb-1 text-left">Anchored Rate</p>
                  <p className="font-black text-white text-sm text-left">${alert.oldPrice.toFixed(2)}</p>
                </div>
                <div className="bg-black/20 p-4 rounded-2xl">
                  <p className="text-[9px] font-black text-slate-500 uppercase mb-1 text-left">Proposed Rate</p>
                  <p className="font-black text-white text-sm text-left">${alert.newPrice.toFixed(2)}</p>
                </div>
              </div>
        ...
        <div className="py-24 text-center">
          ...
          <h4 className="font-black text-white uppercase text-center">Vault Anchors are Stable</h4>
          <p className="text-slate-500 font-bold uppercase text-[10px] mt-2 text-center">No price deviations found in recent audits.</p>
        </div>
      )}
    </div>
    ...
    <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden text-left">
      <div className="p-8 border-b flex justify-between items-center flex-wrap gap-4">
        <div>
          <h3 className="font-black text-slate-900 uppercase text-sm tracking-tight flex items-center text-left">
            <History size={18} className="mr-3 text-slate-400" /> Adjustment Audit Trail
          </h3>
          <p className="text-[10px] font-bold text-slate-400 uppercase mt-1 text-left">Permanent record of all accepted price anchor shifts</p>
        </div>
        <div className="flex items-center space-x-3">
          <button 
            onClick={exportTrail} 
            disabled={acceptedAlertHistory.length === 0}
            className={`flex items-center space-x-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${acceptedAlertHistory.length > 0 ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}
          >
            <Download size={14} /> <span>Extract Trail ({acceptedAlertHistory.length})</span>
          </button>
        </div>
      </div>
            ...
            {acceptedAlertHistory.map((entry: any, idx: number) => (
              <tr key={idx} className="hover:bg-slate-50 transition-colors">
                <td className="px-8 py-6">
                  <p className="text-[11px] font-black text-slate-900 text-left">{entry.date}</p>
                  <p className="text-[9px] font-bold text-slate-400 mt-0.5 uppercase text-left">Logged {new Date(entry.acceptedAt).toLocaleDateString()}</p>
                </td>
                <td className="px-8 py-6">
                  <p className="text-[9px] font-black text-blue-600 uppercase mb-1 text-left">{entry.supplier}</p>
                  <p className="text-[11px] font-black text-slate-900 uppercase text-left">{entry.itemName}</p>
                </td>
                <td className="px-8 py-6">
                  <div className="flex items-center space-x-2">
                    <Hash size={12} className="text-slate-300" />
                    <span className="text-[10px] font-bold text-slate-500 text-left">Inv #{entry.invoiceNumber}</span>
                  </div>
                </td>
                <td className="px-8 py-6 text-right">
                  <p className="text-[11px] font-black text-slate-900 text-right">${entry.newPrice.toFixed(2)}</p>
                  <p className="text-[9px] font-bold text-slate-400 mt-0.5 uppercase text-right">Prev: ${entry.oldPrice.toFixed(2)}</p>
                </td>
                <td className="px-8 py-6 text-right">
            ...
            {acceptedAlertHistory.length === 0 && (
              <tr>
                <td colSpan={6} className="py-24 text-center">
                  <Files size={40} className="mx-auto text-slate-200 mb-4 text-center" />
                  <p className="text-[10px] font-black text-slate-300 uppercase text-center">Audit trail is currently empty.</p>
                </td>
              </tr>
            )}
          </tbody>
           ...
           <tbody className="divide-y divide-slate-100">
              {enrichedInvoices.filter((i: any) => !i.isPaid && !i.isHold).map((inv: any) => (
                <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-6 font-bold text-slate-500 text-[10px] text-left">{inv.date}</td>
                  <td className="px-6 py-6 font-black text-slate-900 text-xs uppercase text-left">{inv.supplierName}</td>
                  <td className="px-6 py-6 font-bold text-slate-400 text-xs text-left">{inv.invoiceNumber}</td>
                  <td className="px-6 py-6 text-right font-black text-slate-900 text-xs text-right">${inv.totalAmount.toFixed(2)}</td>
           ...
           <tbody className="divide-y divide-slate-100">
              {enrichedInvoices.filter((i: any) => i.isPaid).map((inv: any) => (
                <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-6 font-bold text-slate-500 text-[10px] text-left">{inv.date}</td>
                  <td className="px-6 py-6 font-black text-slate-900 text-xs uppercase text-left">{inv.supplierName}</td>
                  <td className="px-6 py-6 text-right font-black text-emerald-600 text-xs text-right">${inv.totalAmount.toFixed(2)}</td>
                  <td className="px-6 py-6 text-center">
...
const HoldView = ({ enrichedInvoices, toggleInvoiceHold, totalOnHold }: any) => (
  <div className="space-y-6">
    <div className="bg-rose-50 border border-rose-100 p-8 rounded-[2rem] flex items-center justify-between">
       <div>
          <h4 className="text-[10px] font-black text-rose-600 uppercase tracking-widest mb-1 text-left">Capital Locked on Hold</h4>
          <p className="text-3xl font-black text-rose-900 text-left">${totalOnHold.toFixed(2)}</p>
       </div>
       <div className="bg-white/50 p-4 rounded-2xl"><Clock className="text-rose-500" size={32} /></div>
    </div>
    <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden text-left">
       <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left min-w-[900px]">
             <tbody className="divide-y divide-slate-100">
                {enrichedInvoices.filter((i: any) => i.isHold).map((inv: any) => (
                  <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-6 font-bold text-slate-500 text-[10px] text-left">{inv.date}</td>
                    <td className="px-6 py-6 font-black text-slate-900 text-xs uppercase text-left">{inv.supplierName}</td>
                    <td className="px-6 py-6 text-right font-black text-rose-600 text-xs text-right">${inv.totalAmount.toFixed(2)}</td>
                    <td className="px-6 py-6 text-center">
                ...
                {enrichedInvoices.filter((i: any) => i.isHold).length === 0 && (
                  <tr>
                    <td className="py-20 text-center">
                      <p className="text-[10px] font-black text-slate-300 uppercase">No invoices currently on hold</p>
                    </td>
                  </tr>
                )}
             </tbody>
...
const MasterStockView = ({ masterItems, selectedStockItems, setSelectedStockItems, exportStockToCSV, deleteMasterItem, searchQuery, setSearchQuery }: any) => {
  const filteredItems = useMemo(() => {
    if (!searchQuery) return masterItems;
    const lowerQuery = searchQuery.toLowerCase();
    return masterItems.filter((item: any) => 
      item.name.toLowerCase().includes(lowerQuery) || 
      item.supplierName.toLowerCase().includes(lowerQuery)
    );
       ...
       <div className="p-6 border-b flex items-center justify-between flex-wrap gap-4">
          <div className="relative w-full max-w-md text-left">
             ...
             <input 
              ...
              className="w-full bg-slate-50 border-none rounded-2xl py-3 pl-12 text-xs font-bold outline-none" 
             />
          </div>
          <button onClick={exportStockToCSV} disabled={selectedStockItems.length === 0} className={`flex items-center space-x-2 px-6 py-3 rounded-2xl text-[10px] font-black uppercase transition-all ${selectedStockItems.length > 0 ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}>
            <FileSpreadsheet size={16} /> <span>Extract CSV ({selectedStockItems.length})</span>
          </button>
       </div>
       <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left min-w-[1000px]">
             <thead>
                <tr className="bg-slate-50 border-b text-[10px] font-black uppercase text-slate-400">
                   <th className="px-6 py-5 w-10"><input type="checkbox" checked={selectedStockItems.length === filteredItems.length && filteredItems.length > 0} onChange={(e) => e.target.checked ? setSelectedStockItems(filteredItems.map((m: any) => m.id)) : setSelectedStockItems([])} className="rounded" /></th>
             ...
             <tbody className="divide-y divide-slate-100">
                {filteredItems.map((item: any) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-6 font-black text-slate-900 text-xs uppercase text-left">{item.name}</td>
                    <td className="px-6 py-6 font-bold text-slate-400 text-[10px] uppercase text-left">{item.supplierName}</td>
                    <td className="px-6 py-6 text-right font-black text-slate-900 text-xs text-right">${item.currentPrice.toFixed(2)}</td>
                ...
                {filteredItems.length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-20 text-center">
                      <p className="text-[10px] font-black text-slate-300 uppercase">No items match your search criteria</p>
                    </td>
                  </tr>
                )}
             </tbody>
...
const VendorsView = ({ suppliers, rawInvoices }: { suppliers: Supplier[], rawInvoices: Invoice[] }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 text-left">
     {suppliers.map((sup) => {
       const outstanding = rawInvoices.filter(inv => inv.supplierName.trim().toUpperCase() === sup.name.trim().toUpperCase() && !inv.isPaid && !inv.isHold).reduce((sum, inv) => sum + inv.totalAmount, 0);
       return (
        <div key={sup.id} className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 space-y-6 hover:shadow-xl hover:border-blue-200 transition-all group relative overflow-hidden">
            <div className="flex items-start justify-between">
              <div className="flex items-center space-x-4">
                  ...
                  <div className="min-w-0">
                      <h3 className="font-black text-slate-900 uppercase text-base tracking-tight truncate max-w-[180px] text-left">{sup.name}</h3>
                      <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-0.5 text-left">Verified Entity</p>
                  </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <p className="text-[9px] font-black text-slate-400 uppercase mb-1 text-left">Total Volume</p>
                    <p className="font-black text-slate-900 text-sm text-left">${sup.totalSpent.toLocaleString()}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <p className="text-[9px] font-black text-slate-400 uppercase mb-1 text-left">Outstanding</p>
                    <p className={`font-black text-sm text-left ${outstanding > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>${outstanding.toFixed(2)}</p>
                </div>
            </div>
               ...
               <div className="p-4 bg-slate-900 rounded-2xl border border-slate-800 text-white">
                  <div className="flex items-start space-x-3 text-left">
                     ...
                     <div className="min-w-0 flex-1 text-left">
                        ...
                        <p className="text-[11px] font-black truncate text-left">{sup.bankAccount || 'Banking details required'}</p>
                     </div>
                  </div>
               </div>
               ...
               <div className="pt-2">
                  <div className="flex items-start space-x-3 text-left">
                     ...
                     <div className="min-w-0 text-left">
                        ...
                        <p className="text-[10px] font-bold text-slate-600 leading-relaxed italic text-left">{sup.address || 'Address information not audited'}</p>
                     </div>
                  </div>
               </div>
            </div>
     ...
     {suppliers.length === 0 && (
       <div className="col-span-full py-24 text-center">
          <Building2 size={64} className="mx-auto text-slate-200 mb-6 text-center" />
          <h4 className="font-black text-slate-400 uppercase text-center">Vendor Intelligence Not Found</h4>
          <p className="text-slate-300 font-bold uppercase text-[10px] mt-2 text-center">Upload invoices to build business profiles.</p>
       </div>
     )}
  </div>
);
        ...
        return (
          <div key={month} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden mb-8 text-left">
            <div className="px-8 py-6 bg-slate-50 border-b flex justify-between items-center">
              <h4 className="font-black text-slate-900 uppercase text-sm text-left">{month} Summary</h4>
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
              {Object.entries(supplierGstMap).map(([name, amount]: any) => (
                <div key={name} className="p-5 bg-slate-50 rounded-2xl border border-slate-100 text-left">
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-2 truncate text-left">{name}</p>
                  <span className="font-black text-slate-900 text-lg text-left">${amount.toFixed(2)}</span>
                </div>
              ))}
            </div>
          </div>
...
const InvoiceViewerModal = ({ selectedInvoice, setSelectedInvoice }: any) => (
  <div className="fixed inset-0 z-[1000] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 text-left">
    <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 text-left">
       <div className="p-8 bg-slate-900 text-white flex justify-between items-start text-left">
          <div className="text-left"><h2 className="text-2xl font-black uppercase tracking-tighter text-left">{selectedInvoice.supplierName}</h2><p className="text-xs font-bold text-blue-400 mt-1 text-left">Invoice #{selectedInvoice.invoiceNumber}</p></div>
       ...
       <div className="p-8 overflow-y-auto custom-scrollbar flex-1 space-y-8 text-left">
          ...
          <div className="space-y-2 text-left">
            {selectedInvoice.items.map((item: any, idx: number) => (
              <div key={idx} className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100 text-left">
                <div className="min-w-0 text-left flex-1"><p className="text-[11px] font-black text-slate-900 uppercase truncate text-left">{item.name}</p><p className="text-[9px] font-bold text-slate-400 uppercase text-left">{item.quantity} units x ${item.unitPrice.toFixed(2)}</p></div>
                <p className="text-[11px] font-black text-slate-900 ml-4 text-right">${item.total.toFixed(2)}</p>
              </div>
            ))}
          </div>
       </div>
...
const UploadView = ({ handleFileUpload, vaultEmail }: any) => (
  <div className="max-w-4xl mx-auto py-20 text-center">
    <div className="w-20 h-20 bg-blue-600 text-white rounded-[2rem] flex items-center justify-center mx-auto mb-10 shadow-2xl text-center"><Upload size={32} /></div>
    <h2 className="text-4xl font-black text-slate-900 uppercase tracking-tighter mb-4 text-center">Ingestion Gateway</h2>
    <p className="text-slate-400 font-bold mb-12 text-[11px] uppercase tracking-widest leading-relaxed text-center">FORWARD PDFs TO: <span className="text-blue-600 lowercase">{vaultEmail}</span></p>
    <label className="group relative block cursor-pointer px-4">
      <div className="border-4 border-dashed border-slate-200 rounded-[3rem] p-24 bg-white transition-all group-hover:border-blue-500 group-hover:bg-blue-50 shadow-sm">
        <Plus size={32} className="mx-auto text-slate-300 group-hover:text-blue-500 mb-4 transition-transform group-hover:scale-125 text-center" />
        <p className="text-slate-400 font-black uppercase text-xs tracking-widest text-center">Auditor Dropbox</p>
      </div>
